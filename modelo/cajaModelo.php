...
class caja
{
    ...
    function listarIngresos()
    {
        ...
        FROM caja
        WHERE accion = 'depositar' AND DATE(created_at) = CURDATE();"; // Consulta SQL para seleccionar todas las categorías
    ...
    {
        $sql = "SELECT id_caja, concepto, accion, monto, created_at
        FROM caja WHERE accion = 'retirar' AND DATE(created_at) = CURDATE();";
        $query = $this->acceso->prepare($sql);
        $query->execute(); // Ejecución de la consulta
        $this->objetos = $query->fetchAll(); // Almacenamiento de los resultados en la propiedad 'objetos'
    ...
    {
        $sql = "SELECT id_caja, concepto, accion, monto, descripcion, created_at
    ...
    function crear_caja($accion, $monto,  $concepto, $id_usuario, $descripcion)
    {
        $sql = "INSERT INTO caja (accion, monto, concepto, id_usuario, descripcion) 
        VALUES (:accion, :monto, :concepto, :id_usuario, :descripcion)";
    ...
    //editar
    function editar_caja($id_caja, $concepto, $accion, $monto, $id_usuario, $descripcion)
    {
        try {
            $sql = "UPDATE caja SET concepto=:concepto, accion=:accion, monto=:monto, id_usuario=:id_usuario, descripcion=:descripcion WHERE id_caja=:id_caja";
            $query = $this->acceso->prepare($sql);
