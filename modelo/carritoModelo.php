...
class carrito
{
    ...
    p.nombre_producto,
		p.precio_producto,
		p.descuento,
    ROUND(p.precio_producto - (p.precio_producto * (p.descuento / 100)), 2) AS precio_con_descuento, -- Precio con descuento redondeado a 2 decimales
    p.stock_producto,
    ct.nombre_categoria,
    SUM(c.cantidad_carrito) AS total_cantidad_carrito,
    ROUND(SUM(c.cantidad_carrito * (p.precio_producto - (p.precio_producto * (p.descuento / 100)))), 2) AS total_valor_carrito, -- Total con descuento
    (
        SELECT url_imagen
        FROM imagen i
